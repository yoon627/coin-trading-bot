package com.trading.common.indicator
...
object Indicators {
    ...
    fun calculateTargetPrice(candles: List<NormalizedCandle>, k: Double = 0.5): Double {
        if (candles.size < 2) return 0.0
        val today = candles[0]
        val yesterday = candles[1]
        val range = yesterday.highPrice - yesterday.lowPrice
        return today.openPrice + range * k
    }
    ...
    fun calculateMa(candles: List<NormalizedCandle>, period: Int): Double {
        if (candles.size < period) return 0.0
        return candles.take(period).map { it.closePrice }.average()
    }

    fun checkGoldenCross(candles: List<NormalizedCandle>, shortPeriod: Int = 5, longPeriod: Int = 20): Boolean {
    ...
    fun isMaUptrend(candles: List<NormalizedCandle>, shortPeriod: Int = 5, longPeriod: Int = 20): Boolean {
    ...
    fun calculateBollingerBands(candles: List<NormalizedCandle>, period: Int = 20, multiplier: Double = 2.0): BollingerBands? {
        if (candles.size < period) return null
        val closes = candles.take(period).map { it.closePrice }
    ...
    fun calculateMacd(
        ...
        val closes = candles.take(needed).map { it.closePrice }.reversed()
    ...
    fun calculateStdDev(candles: List<NormalizedCandle>, period: Int): Double {
        if (candles.size < period) return 0.0
        val closes = candles.take(period).map { it.closePrice }
    ...
    fun calculateEma(candles: List<NormalizedCandle>, period: Int): Double {
        if (candles.size < period) return 0.0
        val closes = candles.take(period).map { it.closePrice }.reversed()
        val k = 2.0 / (period + 1)
