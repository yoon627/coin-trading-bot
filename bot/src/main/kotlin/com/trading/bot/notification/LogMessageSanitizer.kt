...
object LogMessageSanitizer {
    ...
    private val rules: List<Pair<Regex, String>> = listOf(
        ...
        // access_key / secret_key / appkey / appsecret / password = <value> (콜론·등호·공백 구분자, JSON·kv 형태)
        Regex("(?i)(access[_-]?key|secret[_-]?key|app[_-]?key|app[_-]?secret|password)([\"']?\\s*[:=\\s]\\s*[\"']?)([^\"',\\s}]+)") to "$1$2***",
        // KIS 계좌 식별자 (CANO / ACNT_PRDT_CD)
        Regex("(?i)(cano|acnt[_-]?prdt[_-]?cd)([\"']?\\s*[:=\\s]\\s*[\"']?)([^\"',\\s}]+)") to "$1$2***",
