...
커밋된 `bot` 모듈만 단일 EC2 에 올리는 경량 배포. **app + PostgreSQL + Redis** 3개 컨테이너만 사용하고
Kafka/collector/모니터링은 제외한다(미커밋 모듈이며 4GB 박스에서 OOM).
...
- 기본적으로 SSH(22)·앱(8080)은 **본인 공인 IP/32 에만** 열린다(`.env` 미설정 시 자동). 다른 곳에서 접근하려면 `APP_ALLOW_CIDR` 조정.
- 8080 은 평문 HTTP. 외부 공개 시 JWT 가 평문 전송되므로, 공개 접근이 필요하면 **SSH 터널**(`ssh -L 8080:localhost:8080 ...`) 또는 Caddy/Nginx + Let's Encrypt(TLS) 추가 권장.
