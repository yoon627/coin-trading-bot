...
코드 리뷰(버그·보안·클린코드·테스트)는 구현 직후 리뷰 단계 — Claude `code-reviewer` + Codex 병행(크레딧 가용 시) — 가 담당한다. `.git/hooks/pre-push` 는 `deploy.yml` paths-ignore 자기제외 가드만 남았다(codex 게이트는 2026-09-03 제거, `scripts/git-hooks/README.md`).
