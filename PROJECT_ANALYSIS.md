...
│       ├── strategy/                # 스캘핑 전략 (2개) + MlStrategy (MlModelService 의존)
│       ├── ml/                      # 머신러닝 (GBM)
...
│
│   * 참고: 기존 스윙 전략 7개(`VolatilityBreakout`, `RsiBounce`, `GoldenCross`, `MacdCross`,
│     `BollingerBounce`, `MeanReversion`, `CombinedStrategy`) 및 `TradingStrategy`,
│     `TradingProperties`, `Candle`은 `:research`에서 어댑터로 재사용하기 위해 `:common`으로 이동.
│
├── research/                        # (신규 v1) 리서치·백테스트·리포트 전용 모듈
│   └── src/main/kotlin/com/trading/research/
│       ├── engine/                  # 이벤트 드리븐 시뮬레이터 (BarStream, Engine.run, Clock)
...
│       ├── metrics/                 # correct Sharpe/Sortino/Calmar/MaxDD/VaR
...
### 수동 거래 / 포트폴리오 / 전략 / ML / 사용자 / 관심종목
TradingController, PortfolioController, ManualTradeController, StrategyController,
MlController, LeaderboardController(`/api/user/me`/`/api/user/settings`/리더보드),
WatchlistController. 전체 endpoint 목록은 `README.md` 참고.

### 에러 응답 정책
- `SafeErrorAttributes`가 `ResponseStatusException.reason`만 클라이언트에 노출
  (FQCN/스택/메시지 leak 차단).
- `UpbitErrorHandlerAdvice`가 `UpbitApiException` → 사용자 친화적 4xx로 변환.
  특히 raw 401은 절대 노출하지 않음 (FE의 401 자동 logout 회피).
...
11. **리서치-라이브 분리** — `:research`는 `:common`에만 의존, 라이브 엔진(`:bot`) 코드와 격리. 기존 스윙 전략 7개를 `LegacyStrategyAdapter`로 `:research`에서 검증 가능. `MlStrategy`는 `MlModelService` 의존으로 `:bot` 잔존.
