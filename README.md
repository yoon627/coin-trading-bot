...
> 이 프로젝트는 경량화(rightsizing)를 거쳐 단일 인스턴스(EC2 t4g.medium)에서 `app + PostgreSQL + Redis`(+ TLS 종단용 Caddy)로 동작합니다. 구 `:collector`/`:research` 모듈, Kafka 이벤트 백본, ML(Smile), 스캘핑 전략, Claude 분석, Prometheus/Grafana/Loki 모니터링은 제거됐습니다.
...
```
   ...
   └──────────┘            │   │   │   :8080      │   │   :5432     │  │   │
                           │   │   │ (수집+매매   │   └─────────────┘  │   │
   ┌──────────┐   알림     │   │   │  +REST+SPA   │   ┌─────────────┐  │   │
   │ Discord  │◄───────────┼───┤   │   expose)    │──►│   Redis     │  │   │
   │ Webhook  │            │   │   └──────▲───────┘   │   :6379     │  │   │
   └──────────┘            │   │          │ proxy     └─────────────┘  │   │
   ┌──────────┐   HTTPS    │   │   ┌──────┴───────┐                    │   │
   │ Browser  │◄──:443─────┼───┤   │  caddy :443  │  Let's Encrypt TLS │   │
   │  (SPA)   │            │   │   │  (TLS 종단)  │  (sslip.io)        │   │
   └──────────┘            │   │   └──────────────┘                    │   │
                           │   └──────────────────────────────────────┘   │
                           └──────────────────────────────────────────────┘
...
외부 접근은 **Caddy**가 443에서 TLS를 종단(Let's Encrypt 인증서 자동 발급/갱신)하고 내부 `app:8080`으로 리버스 프록시합니다 — `app`은 호스트에 포트를 노출하지 않습니다(`expose`만). prod 프로파일이 항상 발급하는 `Secure` 쿠키가 HTTPS에서 정상 저장돼 로그인이 동작합니다.

### 컨테이너 구성 (`deploy/aws/docker-compose.prod.yml`)

| 서비스 | 이미지 | 역할 |
|--------|--------|------|
| `caddy` | `caddy:2-alpine` | TLS 종단(Let's Encrypt 자동) + 리버스 프록시 → `app:8080` (외부 진입점 :80/:443) |
| `app` | `ghcr.io/yoon627/coin-trading-bot` (GHCR, multi-arch) | Spring Boot 메인 앱 (시세 수집 + 매매 엔진 + REST API + SPA). 호스트 미노출, Caddy 경유만 |
...
| TLS | Caddy 2 + Let's Encrypt | HTTPS 종단(443) + `app:8080` 리버스 프록시, sslip.io 자동 도메인 |
...
단일 EC2 인스턴스에 Docker Compose로 `caddy + app + postgres + redis`를 띄웁니다. CI가 GHCR에 multi-arch 이미지를 push하고, 인스턴스는 그 이미지를 pull 합니다. Caddy가 443에서 HTTPS를 종단(Let's Encrypt 자동 발급)하고 내부 `app:8080`으로 프록시합니다.

```
EC2 t4g.medium (arm64, 4GB, 20GB gp3 EBS)
└── docker compose (deploy/aws/docker-compose.prod.yml)
    ├── caddy (TLS 종단 :80/:443, Let's Encrypt) → app:8080
    ├── app (GHCR 이미지 pull, 내부 :8080 / 호스트 미노출)
    ├── PostgreSQL 17
    └── Redis 7
```
...
> `INSTANCE_TYPE`, `EBS_SIZE_GB`, `SSH_ALLOW_CIDR`, `APP_ALLOW_CIDR`, `APP_DOMAIN`(미설정 시 EC2 공인 IP 기반 sslip.io 자동) 는 `.env`로 조정합니다. `APP_ENCRYPTION_SECRET`은 사용자별 Upbit 키를 복호화하는 AES 키이므로 **한 번 생성된 뒤 변경하면 안 됩니다**.
