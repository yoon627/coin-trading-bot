...
# 이 상한은 불변식이 아니라 tripwire 다 — 걸리면 "왜 늘었는가"를 확인하고 의식적으로 올린다.
# 확인 없이 숫자만 따라 올리면 무한 증식 감지라는 목적이 사라지므로, 올릴 때 이유를 여기 남긴다.
#
# 조정 이력:
#   28±2 → 33±2 (2026-09-01, #152) — DB 통합테스트 하네스 결정·skip 교훈 2페이지(#53) 및
#     그 이전 세션들의 lesson 누적분(rollback-removal 등). 전부 의도한 추가였고 중복·자동생성은 없다.
#   33±2 → 36±2 (2026-09-03) — query/yearly-strategy-comparison 1페이지(운영 8종 1년 전략 비교, 36 페이지). 의도한 추가.
#     36 을 중심으로 둔 것은 상한에 걸친 채로 두면 다음 1페이지에 바로 FAIL 이 나기 때문.
#   39±2 → 40±2 (2026-09-05) — query/trailing-arm-finding-2026-09 1페이지(미관측 7국면 사전고정 확증, 40 페이지).
#     의도한 추가 — exit-resolution-verdict 가 낸 음성 결론과 **다른 claim class**(확증)라 한 페이지에 섞지 않았다.
#   36±2 → 39±2 (2026-09-05) — parameter-search-2026-09 · hold-limit-policy-2026-09 · exit-resolution-verdict-2026-09
#     3페이지(1년 전략 탐색 3개 작업의 산출물, 39 페이지). 앞 둘은 2026-09-04 에 추가돼 상한 38 안이었고,
#     세 번째가 넘겼다. 전부 의도한 추가이며 서로를 정정·인용하는 관계다(중복 아님).
#   40±2 → 43±2 (2026-09-09) — query/trailing-width-2026-09(#184, 2026-09-08) · take-profit-stop-loss-2026-09 · exit-resolution-ladder-2026-09
#     3페이지(청산 축 사전고정 판정 3개 작업의 산출물, 43 페이지). 앞 둘은 상한 42 안이었고 세 번째가 넘겼다.
#     전부 의도한 추가 — 셋은 같은 계기의 편향을 단계적으로 좁힌 결과라 서로를 인용하며, 마지막 페이지가 앞 둘의 통과를 계기 편향으로 정정한다(중복 아님).
#   43±2 → 46±2 (2026-09-16) — query/shared-balance-2026-09 · entry-set-decomposition-2026-09 · entry-resolution-vs-live-2026-09
#     3페이지(#180 공유잔고 재판정, #190 분해와 그 라이브 대조). 앞 둘은 상한 45 안이었고 세 번째가 넘겼다.
#     전부 의도한 추가 — 뒤 둘은 같은 질문(#190)의 분해→실측 순서라 서로를 인용하고, 마지막이 앞 것의 "한계"(라이브 대조 부재)를 닫는다(중복 아님).
if [ "$count" -lt 44 ] || [ "$count" -gt 48 ]; then
  echo "FAIL: 페이지 수 $count 가 기대 범위(46±2) 밖 — 늘어난 이유를 확인하고 이 파일의 조정 이력에 남긴 뒤 올린다"; fail=1
fi
