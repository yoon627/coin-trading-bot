...
`TradeRoundTrip.kt` 는 **추정치가 섞인 그룹에만** 상대 허용 오차를 적용한다.

```
허용오차 = if (수동 증분 있음) max(1e-8, buyVolume × 0.002) else 1e-8
```

- 실측만으로 이뤄진 그룹은 완화하지 않는다 — 완화할 근거가 없다.
- 청산 판정(`isClosed`)과 `oversold` 판정에 **같은 값**을 쓴다. 기준이 갈리면
  "청산됐는데 매수 기록을 못 믿는다" 는 모순 상태가 생긴다.
- **폭의 상한은 계약이 정한다** — `수동 100.0 매수 → 100.3 매도`(초과 0.3%)를 실제 잔여분으로 보고
  손익을 비우는 기존 테스트(`TradeRoundTripTest.kt:261`)가 있다. 넓히면 없는 손익을 만들어낸다.
