...
- **평균은 "관측된 봉"으로 나눈다.** 창이 최대 2일까지 늘어지는 것을 허용하므로(24/7 시장의 드문 결측
  대비), 결측이 있는 종목은 엄밀히는 *30일 평균*이 아니라 *관측 30봉 평균*이다. 결측일을 0 으로 채우면
  추정량이 달라져(거래가 없던 날을 "거래대금 0" 으로 세는 것) 비교 기준이 바뀌므로 그렇게 하지 않았다.
  허용폭이 30일 중 2일이라 순위 영향은 제한적이지만 **0 은 아니다.**
...
5. 2026-09-01, **날짜 검증을 창 전체로 넓히고 t0 미상장을 이력 부족과 분리한 뒤** — 양끝만 보면 중간 봉이
   깨져도 그 거래대금이 평균에 섞였다.

**다섯 다 3단 감쇠·overlap·placebo·top-8 구성이 동일하다.** 즉 그 판정 구멍은 실재했지만 **이 결론은
