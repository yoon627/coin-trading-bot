...
-- created_at 은 timestamp(시간대 없음, 앱이 UTC 로 기록 — DAILY_RESET 매도가 00:00:09 에 찍힌다). 세션 TZ 에 흔들리지 않게 변환 없이 그대로 찍고 리터럴도 naive 로 비교한다.
...
SELECT id, ticker, side, price,
       to_char(created_at, 'YYYY-MM-DD"T"HH24:MI:SS') AS created_utc,
...
WHERE strategy = 'combined'
  ...
  AND created_at >= timestamp '2026-07-14 00:00:00' AND created_at < timestamp '2026-09-15 00:00:00'
