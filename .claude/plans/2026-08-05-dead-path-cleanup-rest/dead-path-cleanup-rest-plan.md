...
## pre-push codex review 4·5차 (2026-08-06) — 설계 재검토로 수렴

4차·5차에서 같은 계열의 지적이 반복됐다(관측 동일성 판정, 저활동 종목의 1h 변화율). **지엽적 대응을 반복하는 것이 신호**여서 근본을 다시 봤다.

| # | finding | 처분 |
|---|---|---|
| P1(5차) | 저활동 종목은 1h 창에 ticker 행이 0~1건이라 `change_1h` 가 null — 구 `PriceCollector` 는 5분 주기로 거래량과 무관하게 기록했으므로 **외부 계약의 가시적 회귀** | **fix(설계 변경)** — 기준가를 `market_tickers` 가 아니라 **`market_candles`(1분봉)** 에서 얻는다. 캔들은 60초 REST 폴링이라 거래량과 무관하게 채워진다. 샘플링 밀도 문제 자체가 사라진다 |
| P2(4·5차) | 관측 동일성을 시각/행 id 로 판정하는 로직이 계속 구멍을 남김(같은 ms, 메모리 값의 id 부재) | **해소** — 캔들 기준가는 ticker 행과 별개 소스라 "같은 관측" 개념이 성립하지 않는다. `findOldestInRange` 는 죽은 코드가 되어 제거 |

**교훈**: 샘플링된 스트림 저장소로 시간창 지표를 만들려 한 것이 원인이었다. 밀도가 보장된 소스(캔들)를 쓰면 파생 문제 전부가 사라진다.

## pre-push codex review 6~10차 (2026-08-11) — 신선도 요구가 왕복해 사용자 판정

| # | finding | 처분 |
|---|---|---|
| P1(6·7차) | 폴백에 신선도 제한이 없어 며칠 전 가격이 현재가로 표시된다 | **fix 후 되돌림** — 메모리·DB 양쪽에 1h 제한을 걸었더니 10차에서 정반대 지적을 받았다 |
| P2(8차) | `Flux.next()` 는 SQL `LIMIT` 을 걸지 않아 1시간치 60행을 읽는다 | **fix** — `findOldestInRange(candle)` 전용 쿼리 |
| P2(9차) | 창의 첫 봉이 사실상 현재 봉이면 1h 변화율이 아니다 | **fix** — 기준봉 최소 경과(30분) 요구 |
| P1(10차) | **신선도 제한 때문에 조용한 종목이 목록에서 빠진다** — 6·7차와 정면 충돌 | **사용자 판정 → 목록은 항상 표시** — 값이 있으면 오래됐어도 노출하고 `updated_at` 이 시각을 드러낸다. 낡은 값으로 변화율을 만들지 않는 건 기준봉 조건(9차)이 이미 맡는다. 구 `PriceCollector` 동작(5분 주기 기록이라 종목이 사라지지 않음)에 가장 가깝다 |

## pre-push codex review 11~13차 (2026-08-11)

| # | finding | 처분 |
|---|---|---|
| P2(11차) | DB 조회 실패가 바깥 catch 로 전파돼 종목이 통째로 빠진다 — WS 는 멀쩡한데 | **fix** — ticker 이력·기준봉 조회를 각각 감싸 실패를 흡수(`CancellationException` 은 재전파) |
| P2(12차) | 현재값이 1시간보다 오래됐는데 기준봉만 최근이면 엉뚱한 변화율 | **fix** — `latest.at` 도 신선할 때만 계산. 10차 결정(목록엔 남김)과 정합 |
| P2(13차) | 현재값이 50분 전이면 실제로는 10분 구간을 `change_1h` 로 표시 | **wontfix(사용자 판정)** — 구 `price_snapshots` 구현도 창의 최신/최오래를 비교해 **같은 근사**를 갖고 있었다(회귀 아님). 더 엄밀하게 고치면 "현재값이 아주 최신일 때만" 이 되어 10차 결정(조용한 종목에도 변화율 제공)과 충돌한다. `CODEX_ACK=1` 로 진행하고 여기 기록 |

**workflow 관찰**: 3차에서 설계를 바꿔 근본을 해소한 뒤에도 지적마다 즉시 반영하며 7차례를 더 돌았다. 성능·엄밀성 지적(8·9차)은 회귀가 아니었으므로 더 일찍 멈추고 판단을 구했어야 한다.
