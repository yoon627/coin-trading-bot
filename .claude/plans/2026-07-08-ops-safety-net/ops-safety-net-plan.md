...
- 2026-07-17: **O2 DB 백업** 구현(사용자 "이어서"). `deploy/aws/backup.sh` 신규(pg_dump `docker compose exec -T`(unix socket, local trust)+`</dev/null`→gzip→`aws s3 cp - --sse` + RETENTION 경과 객체 정리). `deploy.sh`: render_server_env 에 BACKUP_S3_* + AWS_REGION 추가, do_deploy 가 backup.sh scp+chmod, do_destroy 에 삭제 전 최종백업 훅(BACKUP_S3_BUCKET 설정 시). `.env.example` 백업 섹션(APP_ENCRYPTION_SECRET 오프사이트 분리 경고). 보안: dump 만 백업(AES 키 분리). `bash -n` 통과(backup.sh·deploy.sh). pg_dump/S3/cron 실행은 EC2 핸드오프. 
- 2026-07-17: ops-safety-net 을 origin/main(a44782a — marketdata-consolidation 머지+README refresh 포함)에 **rebase**(5커밋 무충돌 replay, deploy/·Dockerfile 미겹침 확인). refresh 된 `deploy/aws/README.md` 에 O2 백업/복원 섹션 + O4 SHA고정/자동롤백/CI대기 문서화(섹션 3 신설, 4~6 재번호), 상단 README CI 다이어그램의 "latest image" → SHA/롤백으로 정합. bash -n(deploy.sh·backup.sh) 재확인.
