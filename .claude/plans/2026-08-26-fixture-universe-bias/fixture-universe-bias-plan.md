...
- **fix loop 상한 초과 — 최종 9라운드** (dlc 상한 2). 3라운드 이후 지적이 "Upbit 이 실제로 내지 않는
  malformed 응답" 처리의 정교화로 수렴했고, 고칠 때마다 provenance 커밋이 뒤처져 그게 다시 finding 이 되는
  **순환**이 생겼다(코드 확정 → 재실행 → 문서만 갱신 순서로 끊었다).
  값싸고 원칙에 맞는 것은 전부 고쳤다(6건). **마지막 3건은 명시적 risk accept**:
  - 평균을 관측 봉 수로 나눔 — 결측일 0 채우기는 추정량을 바꾼다(위 항목)
  - `fills200` 입력 스냅샷 미보존 — 200봉 커버 판정은 boolean 이고 결과가 5회 동일해 재현 가치가 낮다
  - 200봉 사이 장기 공백 미검증 — 양끝 커버 + 200개 존재로 이미 걸러지고, 실제 fixture 12개는
    전부 동일 구간임이 확인돼 있다
  **판단 근거**: 이 하네스는 CI 비실행·수동 전용이고, 결론(3단 감쇠·overlap·placebo·top-8)이 **5회 실행
  전부 동일**해 어떤 판정 구멍에도 의존하지 않음이 실측됐다. 더 정교화해도 결론이 바뀌지 않는다.
