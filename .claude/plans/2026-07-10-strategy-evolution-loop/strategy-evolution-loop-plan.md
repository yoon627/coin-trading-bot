...
- **Phase 0 = 정합 게이트 선행(루프 신뢰의 전제)**: ① #31 신호 파라미터 config 분리, ② #33 포함 **intrabar 보수 청산 모델** — 백테 processExit 가 봉 종가에서만 SL/TP/trailing 평가(BacktestEngine.kt:113-117), 라이브는 10초 tick — 모든 가격 게이트가 체계적으로 낙관. SL 은 low, TP 는 high 판정 (2026-07-18 plan-review 교정: **순서 TRAILING→SL→TP**, trailing pnlPct 게이트는 **트레일링 체결선 기준**, SL↔TP 동시충족만 SL 우선(worst-case), 체결가=게이트 임계선(threshold), **TIME_EXIT=현재봉 open**+한도봉 intraday 미평가 — 이유: 라이브 SL우선은 tick 상호배타 전제라 봉붕괴 모델에 그대로 옮기면 이익 트레일링 거래를 손절로 오기록; Upbit D1 봉 open=09:00 KST 리셋 시장가 researcher 확인), ③ **M1 replay 편향 실측** — M1 30일 보존(DataRetentionService.kt:21) 활용해 D1 청산 모델 편향을 정량화. 단 30일 창의 청산 이벤트가 한 자릿수일 수 있음 — 리포트에 **이벤트 수·불확실성 병기, 표본 미달 시 판정 유보**(공허한 게이트 방지), ④ engine_version 기록(시맨틱 변경 시 과거 run 비교 차단 + champion 재검증 강제). (이유: 모델 오차 > 표본 오차인 동안 통계 게이트는 장식 — 2026-06 스윕 arm 축 변별 불가로 실증)
...
## #33 (2026-07-18, plan-reviewer + code-reviewer Claude + codex high 병행, REQUEST CHANGES→fix)
- **fix(Major)**: 같은 봉 high 로 트레일링 arm 되는 낙관 편향(SL 손실을 팬텀 트레일링 이익으로 오기록, #33 보수화 목표 역행) — 트레일링 arm peak 을 **이 봉 high 반영 전 peak(직전 고점)**으로 분리. 팬텀 트레일링 재현 테스트로 가드.
- **fix(Minor)**: 한도봉 chartExit 종가노출 누수(open-only 불변식 우회) — `!atHoldLimit` 가드로 한도봉 chartExit 스킵(라이브는 09:00 리셋 시 그 봉 종가 미형성).
- **defer**: 한도봉 chartExit 전용 테스트 — chartExitEnabled 기본 false 라 미노출 + 시나리오 구성 복잡. 로직 수정은 반영, 전용 테스트는 후속.
