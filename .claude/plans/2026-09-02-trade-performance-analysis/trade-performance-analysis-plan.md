...
**사용자 결정 대기 — 파라미터는 아직 손대지 않았다(전제 유지).** 선택지:

1. **소액 실거래 실험: `TRADING_MAX_HOLD_DAYS` 상향**(코드 0줄, [[reset-churn-measurement]] 가 남긴 두 경로 중 하나).
   `.env` 만 바꾸면 되지만 [[feedback_config_defaults_multi_layer]] 대로 코드·compose·`.env` 3계층 확인 후 재배포·로그 검증.
   기간·판정 기준(예: 30건 후 사유별 EV 재조회)을 먼저 고정한다.
2. **관망 + 표본 축적** — n=42(리셋 25)는 평균 손익의 표준오차가 ±0.4~0.5%p 라 지금 결론 대부분이 잡음 범위다.
3. **M1 fixture 도입(#143)** 후 백테로 리셋 정책 재검증 — 가장 정직하지만 가장 비싸다.

어느 쪽이든 `BacktestEngine` 근거 없이 TP/SL 숫자를 바꾸는 것은 하지 않는다(Acceptance 4).
...
- 쿼리 [10] 의 max_hold 78h/85h 가 `maxHoldDays=1` 과 모순 — BUY 매칭(수동 BUY 포함)의 문제인지 리셋 누락인지 미확인. (중간·조사)
