...
**2차 code-review 대응 완료(Critical 1 + Major 3 + Minor 3), 전체 470 test green.** 다음: **3차 리뷰**(2차에서 확인된 결함이 실거래 자금 손실급이었으므로 재확인 필요) → simplify 체크 → push/PR(`Closes #19, #20`). 미처리 지적의 처분은 아래 `# Review Disposition` 2026-07-26 절 참조. 배포 전 절차는 Blockers 의 V14 항목. 재현 테스트는 실제 restart 경로(엔진 레벨) 커버 추가.
