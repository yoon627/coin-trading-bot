...
**Critical 5건 + Major 7건 fix 완료, 전체 467 test green.** 다음: **code-review 재실행**(메인 code-reviewer + codex 병행 — 이번 fix 12건이 리뷰 미검증). 통과 후 simplify 체크 → push/PR(`Closes #19, #20`). 배포 전 절차는 아래 Blockers 의 V14 항목 참조. 재현 테스트는 실제 restart 경로(엔진 레벨) 커버 추가.
