...
1. 개정 plan 을 **커밋**한다(사전고정의 감사 근거 — 실행 전 커밋이어야 의미가 있다).
2. TDD Red: `SwingMetrics` 추출 + 게이트 순수함수(G1·G2·G4~G7 / G3 plateau) + 신호 캐시 동치성(마켓 충돌 재현 포함) + null 대조군 순수성.
3. Stage A 구현 → synthetic 벤치마크(성과 수치 미산출)로 런타임 확정 → 사전고정 그리드 실행.
