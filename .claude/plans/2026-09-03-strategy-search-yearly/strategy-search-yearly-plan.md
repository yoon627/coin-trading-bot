...
- 2026-09-04 — Stage B(`5c24760`): 프로덕션 노브 3종을 4중 가드와 함께 추가(`Indicators.calculateAtr` · `BacktestConfig`
  ...
  wiki 검증 3종 통과.
- 2026-09-04 — Stage D(ultracode 워크플로 10 에이전트: 구현 5 + 교차검증 5). 신규 지표 진입 전략 10종 구현
  (ADX/DMI·돈치안·켈트너·슈퍼트렌드·스토캐스틱·Williams %R·OBV·MFI·CCI·VWAP). 교차검증에서 수식·look-ahead·계약 실패 0건,
  실제 결함 2건 수정(VWAP 폴백이 윈도 합계 기준이라 부분 거래대금 창에서 과소계산 / MFI 완전 평탄 구간의 중립값 50 이
  과매도 상향돌파로 읽힘) + 사양 고정 테스트 추가(`StrategySpecPinTest` — "파라미터를 아무 값으로 바꿔도 통과" 지적 대응).
  측정: 39,600 좌표 → 고유 13,580 → **G1 에서 전멸(통과 0)**. 근접 후보는 `obv_trend` TP8/SL5/트레일1.5/h5 로
  선택창 **+5.04%p** 인데 양수 마켓 **5/8** 이라 마켓 다수결(6/8)에서 탈락 — 효과 크기는 있었으나 마켓 간 일관성이 없었다.
...
## 15) Stage D — 신규 지표 기반 진입 전략 (사전고정, 2026-09-04 추가)

Stage A 는 **기존 전략 9종의 청산·kValue** 만 뒤졌다. 진입 신호 자체를 바꾸는 축이 남아 있고, 사용자 요청(2026-09-04)으로
그 축을 연다. 지표는 repo 에 **없는 것**만 고른다(현재 있는 것: RSI·MA 교차·볼린저·MACD·변동성돌파·평균회귀·무릎 2종·ATR).

**후보 10종** — 전부 D1 · 50봉 window 안에서 계산 가능해야 한다(엔진이 넘기는 window 가 정확히 50봉이고
`StrategyMinCandlesTest` 가 `minCandles ≤ 50` 을 강제한다):
ADX/DMI · 돈치안 채널 돌파 · 켈트너 채널 · 슈퍼트렌드 · 스토캐스틱 · Williams %R · OBV · MFI · CCI · VWAP 밴드.

**게이트는 G1~G7 그대로.** 바뀌는 것은 다중비교 폭 하나다:
- 실제 탐색 폭 = (기존 13조합 + 신규 10전략 × kValue 1) = **23조합** × exit 3,960.
- 따라서 null max-statistic 환산 분위를 `q(0.95^(1/13))` 에서 **`q(0.95^(1/23))`** 로 바꾼다(seed 목록 1..91 은 그대로).
- 이 환산은 **결과를 보기 전에** 이 문단으로 고정한다. 신규 전략이 통과해도 이 임계를 못 넘으면 발견으로 보고하지 않는다.

**전략 등록 경계**: 신규 전략은 `YearlyStrategyComparison.ALL_STRATEGIES` 에 넣지 않는다 — 그 목록은 선행 측정
([[yearly-strategy-comparison]])의 모집단이라 바꾸면 그 리포트가 조용히 달라진다. 스윕 전용 목록을 따로 둔다.

**구현 규약**: 지표 계산은 각 전략 파일 안에 private 으로 둔다(공유 `Indicators.kt` 동시 편집 충돌 방지). 라이브 등록은
하지 않는다 — 백테 탐색 전용이며 승격은 사람 승인이다.
